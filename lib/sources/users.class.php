	...
	function displayAdminListFunctions() {
		...
		echo
			"<input type='submit' name='activatesubmit' value='" .
				htmlchars(__("Activate"), ENT_QUOTES) .
				"' class='button' /> " .
			"<input type='submit' name='suspendsubmit' value='" .
				htmlchars(__("Suspend"), ENT_QUOTES) .
		...
		if (isset($GLOBALS['ADMIN']) && (bool)$GLOBALS['ADMIN'])
			...
			$result =
				"<span class='user-name'>" .
					sprintf($format,
						"<a class='user-name-link' href='".
							htmlchars($row['Website'], ENT_QUOTES).
		...
		echo
			"<div class='user-avatar'>" .
				"<img src='http://www.gravatar.com/avatar/".$hash."?s=".$size .
					"&amp;d=".urlencode($default)."' " .
					"border='0' alt='".htmlchars(__("Avatar"), ENT_QUOTES)."' " .
					"class='gavatar' />" .
			"</div>";
								...
								($multiple?
									"onclick=\"" .
										"$('".$target."').val(" .
											"$('".$target."').val()+" .
											"($('".$target."').val()?'" .
												htmlchars($separator, ENT_QUOTES)." ':'')+" .
											"'".$formatedvalue."');" .
