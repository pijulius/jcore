	...
	function displayIcon(&$row) {
		...
		if ($row['_IsDir']) {
			echo
				"<a href='".$row['_Link']."' " .
					"title='" .
						htmlchars(sprintf(__("Change directory to %s"),
			...
			if ($this->picturesPreview && $row['_MimeType'] == 'mime-type-photo') {
				echo
					"<a href='".$row['_ViewLink']."' " .
						...
						"title='".htmlchars($row['_File'], ENT_QUOTES)."' " .
						"class='attachment-preview'>" .
						"<img src='" .$row['_ViewLink']."' " .
							"width='32' height='32' border='0' " .
							"alt='".htmlchars($row['_File'], ENT_QUOTES)."' " .
							"title='" .
								htmlchars(sprintf(__("Preview %s"),
									$row['_File']), ENT_QUOTES) .
							"' />" .
					"</a>";
		...
		if ($row['_IsDir']) {
			...
			if ($this->directLinks) {
				echo
					"<a href='".$row['_URL']."' " .
						"title='" .
							htmlchars(sprintf(__("Link to %s"),
								$row['_File']), ENT_QUOTES) .
						"' " .
						"target='_blank'>" .
						$row['_File'] .
