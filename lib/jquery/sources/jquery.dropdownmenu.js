	...
	dropDownMenu: function(){
		return this.each(function() {
			...
			jtag.mouseenter(function() {
				...
				var submenu = jQuery(this).children('ul,div');
				...
				if (submenu.is(":animated")) {
					...
					submenu.animate({ delay:1 }, 100, function() {
							jQuery(this).animate({height: 'show', opacity: 'show'}, 'fast');
						});
				}
			});
			...
			jtag.mouseleave(function() {
				...
				if (submenu.is(":animated")) {
					...
					submenu.animate({ delay:1 }, 100, function() {
							jQuery(this).animate({height: 'hide', opacity: 'hide'}, 'slow');
						});
				}
			});
		});
