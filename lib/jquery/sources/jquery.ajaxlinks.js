jQuery.ajaxParseScript = function (data, target) {
	scripts = data.match(/<script.*?>.*?<\/script>/gi);
	
	for (var i in scripts) {
		jscript = jQuery(scripts[i]);
		if (jscript.attr('src')) {
			var rscript = document.createElement('script');
			rscript.type  = 'text/javascript';
			rscript.src = jscript.attr('src');
			
			if (typeof(target) == 'object')
				target.append(rscript);
			else
	...
	jQuery(this).click(function(){
		...
		if (jthis.attr('target')) {
			...
			jQuery.get(url, function(data){
				...
				jQuery.ajaxParseScript(data, target);
			});
